...
router.get('/imgTitle/limit/:limit', function(req, res, next) {
    ...
    mysql.select('select id from node_img  limit 0,10', [limit]).spread((response: any) => {
...
router.get('/imgDetail/title/:title', function(req, res, next) {
    ...
    mysql.select('select i.url,t.title  from node_img as i left join node_title as t on t.id=i.titleId  where  i.titleId=?', [req.params.title]).spread((response: any) => {
...
function addTotal(titleId){
    ...
    mysql.query("UPDATE `node_title` SET `total`=total+1 WHERE  id=?", [titleId]).spread((response: any) => {
